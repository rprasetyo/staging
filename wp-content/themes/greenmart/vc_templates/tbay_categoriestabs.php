        ...
        <div class="widget-content woocommerce">
            <ul role="tablist" class="nav nav-tabs">
                <?php foreach ($categoriestabs as $tab) : ?>
                    ...
                    <li <?php echo trim($class_li); ?>>
                        <a href="#tab-<?php echo esc_attr($_id);?>-<?php echo esc_attr($i); ?>" data-toggle="tab" class="tbay-image-loaded <?php echo (isset($tab['icon']) || isset($tab['icon_font']) ? 'has-icon' : 'no-icon'); ?>">
                            <?php if ( isset($tab['icon']) && !empty($tab['icon']) ): ?>
                                <?php $img = wp_get_attachment_image_src($tab['icon'], 'full'); ?>
                                <?php if ( isset($img[0]) ) { ?>

                                    <?php 
                                        $image_alt  = get_post_meta( $tab['icon'], '_wp_attachment_image_alt', true);
            ...
            <div class="widget-inner">
                <?php if( !empty($image_cat) ) : ?>
                    ...
                    <div class="col-lg-3 tbay-image-loaded hidden-md hidden-sm hidden-xs <?php echo esc_attr( $image_float );?>">
                        <?php 
                    ...
                    <div class="tab-content">
                        <?php $i = 0; foreach ($categoriestabs as $tab) : ?>
                            ...
                            if( !in_array($tab['category'], $cat_array_id) ) {
                                ...
                                $class_tab = ($i == 0 ? 'active' : '');
                            ?>

                            <div id="tab-<?php echo esc_attr($_id);?>-<?php echo esc_attr($i); ?>" class="tab-pane <?php echo esc_attr( $class_tab ); ?>">
