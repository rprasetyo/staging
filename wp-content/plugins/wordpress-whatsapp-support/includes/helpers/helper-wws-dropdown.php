...
if (!function_exists('wws_time_hours_dropdown')) {
    ...
    function wws_time_hours_dropdown($attr) {
        ...
        $selected = ( $attr['selected'] ) ? $attr['selected'] : '';
...
if ( ! function_exists( 'wws_admin_page_dropdown' ) ) {

    function wws_admin_page_dropdown( $args ) {
        ...
        echo '<select '.$name .' id="" class="wecreativez-multi-select regular-text" '. $multiple .' >';
...
if ( ! function_exists( 'wws_products_dropdown' ) ) {

    function wws_products_dropdown( $args ) {
        
        global $wpdb;

        $query      = $wpdb->get_results( "SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type = 'product'", ARRAY_A );
        $name       = ( $args['name'] ) ? 'name="' . $args['name'] . '" ' : '';
        $multiple   = ( isset( $args['multiple'] ) ) ? 'multiple' : '';
        ...
        foreach ( $query as $key => $value ) {
            if ( $args['selected'] ) {
                ...
                if ( $multiple ) {
                    if ( in_array( $value['ID'], $args['selected']) ) {
                        $selected = 'selected="selected"';
                    } else {
                        $selected = '';
                    }
                } else {
                    if ( $value['ID'] == $args['selected'] ) {
                        $selected = 'selected="selected"';
                    } else {
                        $selected = '';
                    }
...
if ( ! function_exists( 'wws_categories_dropdown' ) ) {

    function wws_categories_dropdown( $args ) {
        ...
        foreach ( $query as $key => $value ) {
            if ( $args['selected'] ) {
                ...
                if ( $multiple ) {
                    if ( in_array( $value->slug, $args['selected']) ) {
                        $selected = 'selected="selected"';
                    } else {
                        $selected = '';
                    }
                } else {
                    if ( $value->slug == $args['selected'] ) {
                        $selected = 'selected="selected"';
                    } else {
                        $selected = '';
                    }
